defmodule GraffitiTest do
  ...
  setup do
    ...
    # Define schema
    state =
      Graffiti.define_vertex_type(state, :person,
  ...
  describe "basic transaction operations" do
    test "successful transaction", %{state: state} do
      ...
      # Add vertices
      {:ok, state} =
        Graffiti.add_vertex(state, tx_id, :person, "1", %{
          name: "Alice",
          age: 30
        })

      {:ok, state} =
        Graffiti.add_vertex(state, tx_id, :person, "2", %{
          name: "Bob",
          age: 25
        })

      # Add edge
      {:ok, state} = Graffiti.add_edge(state, tx_id, "1", "2", :knows)

      # Commit transaction
      {result, state} = Graffiti.commit_transaction(state, tx_id)
    ...
    test "transaction rollback", %{state: state} do
      ...
      {reason, state} = Graffiti.rollback_transaction(state, tx_id)

      # Verify graph is unchanged
      assert PropGraph.vertex_count(state.graph) == 0
    end
    ...
    test "concurrent transactions", %{state: state} do
      ...
      {tx1, state} = Graffiti.begin_transaction(state)
      {tx2, state} = Graffiti.begin_transaction(state)

      # First transaction acquires lock
      {:ok, state} =
        Graffiti.add_vertex(state, tx1, :person, "1", %{
          name: "Alice",
          age: 30
        })
      ...
      {:error, reason, _state} =
        Graffiti.add_vertex(state, tx2, :person, "1", %{
          name: "Bob",
          age: 25
        })
  ...
  describe "advanced transaction operations" do
    ...
    test "prevents adding edges to non-existent vertices", %{state: state} do
      ...
      {:error, reason, _state} = Graffiti.add_edge(state, tx_id, "1", "2", :knows)
    ...
    test "handles multiple transactions with vertex and edge operations", %{state: state} do
      ...
      {_result, state} = Graffiti.commit_transaction(state, tx1)

      # Second transaction
      {tx2, state} = Graffiti.begin_transaction(state)

      {:ok, state} =
        Graffiti.add_vertex(state, tx2, :person, "2", %{
          name: "Bob",
          age: 25
        })

      {:ok, state} = Graffiti.add_edge(state, tx2, "1", "2", :knows)
      {result, _state} = Graffiti.commit_transaction(state, tx2)
    ...
    test "query operations with committed data", %{state: state} do
      ...
      {:ok, state} = Graffiti.add_edge(state, tx_id, "1", "2", :knows)
      {_result, state} = Graffiti.commit_transaction(state, tx_id)

      # Test query
      {results, _state} = Graffiti.query(state, [:person, :knows, :person])
  ...
  describe "advanced query operations" do
    setup %{state: state} do
      ...
      state =
        Graffiti.define_vertex_type(state, :company,
          name: [type: :string, required: true],
          industry: [type: :string, required: true]
        )

      state =
        Graffiti.define_vertex_type(state, :project,
          name: [type: :string, required: true],
      ...
      # Add vertices
      {:ok, state} =
        Graffiti.add_vertex(state, tx_id, :person, "p1", %{
          name: "Alice",
          age: 30,
          role: "Developer"
        })
      ...
      {:ok, state} =
        Graffiti.add_vertex(state, tx_id, :person, "p2", %{
          name: "Bob",
          age: 25,
          role: "Manager"
        })
      ...
      {:ok, state} =
        Graffiti.add_vertex(state, tx_id, :company, "c1", %{
          name: "TechCorp",
          industry: "Software"
        })

      {:ok, state} =
        Graffiti.add_vertex(state, tx_id, :project, "proj1", %{
          name: "GraphDB",
          status: "Active"
        })

      # Add various relationship types
      {:ok, state} = Graffiti.add_edge(state, tx_id, "p1", "p2", :knows)
      {:ok, state} = Graffiti.add_edge(state, tx_id, "p1", "c1", :works_at)
      {:ok, state} = Graffiti.add_edge(state, tx_id, "p2", "c1", :works_at)
      {:ok, state} = Graffiti.add_edge(state, tx_id, "p1", "proj1", :works_on)
      {:ok, state} = Graffiti.add_edge(state, tx_id, "c1", "proj1", :owns)
    ...
    test "queries with different vertex types", %{state: state} do
      ...
      {results, _} = Graffiti.query(state, [:person, :knows, :person])
      assert length(results) == 1
      assert Enum.member?(results, {"p1", :knows, "p2"})

      # Test person-works_at-company relationship
      {results, _} = Graffiti.query(state, [:person, :works_at, :company])
      ...
      {results, _} = Graffiti.query(state, [:company, :owns, :project])
      assert length(results) == 1
    ...
    test "queries with non-existent patterns", %{state: state} do
      ...
      {results, _} = Graffiti.query(state, [:person, :reports_to, :person])
      assert results == []
